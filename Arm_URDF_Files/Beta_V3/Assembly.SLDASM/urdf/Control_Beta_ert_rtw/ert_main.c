...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(Control_Beta_M, 0);
  ...
  Control_Beta_initialize();
  cli();
  sei ();

  /* External Mode initialization */
  errorCode = extmodeInit(Control_Beta_M->extModeInfo, &rtmGetTFinal
    (Control_Beta_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Control_Beta_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(Control_Beta_M);
